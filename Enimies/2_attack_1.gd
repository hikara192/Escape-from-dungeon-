...
var state_durations = {
	"initial_waiting": 60.0,  # Ждем 60 секунд перед началом первой атаки
	"waiting": 3.0,           # Ожидание между циклами
	"attacking": 10.0,        # Длительность атаки
	"returning": 5.0          # Время возврата
}
...
func _ready() -> void:
	...
	# Сохраняем ссылку на родительскую стену
	...
	print("Первая атака: Ждем 60 секунд перед началом")
	...
	match current_state:
		"initial_waiting":
			if state_machine_timer >= state_durations.initial_waiting:
				_enter_wait_state()  # Переходим к обычному ожиданию
				print("Первая атака: Начинаем цикл атак")
...
func _enter_wait_state():
	print("Первая атака: Ожидание (3 сек)")
...
func _enter_attack_state():
	print("Первая атака: Начало атаки")
	current_state = "attacking"
	state_machine_timer = 0
	is_chasing = true
	
	# Обновляем локальную позицию при начале атаки
	original_local_position = position
...
func _enter_return_state():
	print("Первая атака: Возвращение")
	current_state = "returning"
	state_machine_timer = 0
	is_chasing = false
	
	# Возвращаемся к локальной позиции относительно родителя
